...
def test_dependent_required_property_names_preserved():
    """dependentRequired values are property-name strings, not schemas.

    GitHub Copilot MCP tools ship maps like {\"owner\": [\"repo\"]}. Recursing
    into those lists rewrote the strings into empty object schemas and
    caused provider HTTP 400 schema validation failures (#64587).
    """
    dep = {"owner": ["repo"], "repo": ["owner"]}
    tools = [_tool("mcp_github_search_issues", {
        "type": "object",
        "properties": {
            "owner": {"type": "string"},
            "repo": {"type": "string"},
        ...
        "required": ["owner"],
        "dependentRequired": dep,
    })]
    out = sanitize_tool_schemas(tools)
    params = out[0]["function"]["parameters"]
    assert params["dependentRequired"] == dep
    # Must remain strings — not {"type": "object", "properties": {}}.
    assert params["dependentRequired"]["owner"] == ["repo"]
    assert params["dependentRequired"]["repo"] == ["owner"]
    assert all(isinstance(n, str) for names in params["dependentRequired"].values() for n in names)
